...
class FusionRetrModelBase(nn.Module):
    def __init__(self):
        ...
        self.feat_l1 = self.create_linear_layer(D, D)
    ...
    def get_table_aggr_states(self, batch_data, batch_input_states, 
        ...
        batch_p_table_states = self.table_fnt(batch_input_states, 
                                              sample=sample, calculate_log_probs=calculate_log_probs) 
        bsz, num_layers, _, num_feature_2 = batch_p_table_states.shape
        ...
        for idx, item in enumerate(batch_data):
            ...
            batch_table_lst.append(p_table_lst)
            item_passage_feature = p_aggr_features #torch.cat([passage_states, p_aggr_features], dim=-1)
        ...
        return item_passage_feature_lst, batch_table_lst 
    ...
    def forward(self, batch_data, fusion_scores, fusion_states, passage_masks, 
        ...
        batch_aggr_feature_lst, batch_table_lst = self.get_table_aggr_states(batch_data, input_states, 
        ...
        batch_table_scores = []
        opts['batch_table_lst'] = batch_table_lst
        for idx in range(len(batch_data)):
            aggr_features = batch_aggr_feature_lst[idx]
            item_table_scores = self.feature_fnt(aggr_features,
                                                 sample=sample, calculate_log_probs=calculate_log_probs).squeeze(-1)
            item_table_lst = batch_table_lst[idx]
            item_num_tables = len(item_table_lst)
            table_scores = item_table_scores.view(n_layers, item_num_tables, -1)
            table_sum_scores = table_scores.sum(dim=[0,2])
            batch_table_scores.append(table_sum_scores)  
