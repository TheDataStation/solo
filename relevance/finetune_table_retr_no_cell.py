...
def write_predictions(f_o_pred, item, top_tables, scores):
    qid = item['qid']
...
def get_batch_answers(batch_data, opts):
    batch_table_lst = opts['batch_table_lst']
    batch_answers = []
    for idx, item in enumerate(batch_data):
        table_id_lst = item['table_id_lst']
        item_answers = []
        for table_id in batch_table_lst[idx]:
            label = int(table_id in table_id_lst)
            answer_info = {
                'table_id':table_id,
                'em':label
            }
            item_answers.append(answer_info)
        batch_answers.append(item_answers)
