...
def update_min_tables(item, top_n, max_ctx_per_table):
    #assert(top_n >= 25)
...
def process_train(train_data, top_n, table_dict, strategy, min_tables, max_ctx_per_table=5):
    updated_train_data = []
    for item in tqdm(train_data):
        #pos_lst, neg_lst = collect_passages(item)
        update_min_tables(item, top_n, max_ctx_per_table)
        gold_table_lst = item['table_id_lst']
        ctxs = item['ctxs']
...
def process_dev(dev_data, top_n, table_dict, strategy, min_tables, max_ctx_per_table=5):
    updated_dev_data = []
